...
def run_benchmark_sweep(
    ...
    *,
    ...
    if any(m != SWEEP_MODE_AUTOREGRESSIVE for m in modes_canon):
        raise NotImplementedError(
            "Legacy benchmark sweep modes (speculative, sparse, quant) are archived under "
            "old_impl/mlsys_kv. Use only modes: [autoregressive] with the active stack, or run "
            "PYTHONPATH=old_impl python -m mlsys_kv.cli benchmark-sweep "
            "--config old_impl/configs/<your>.yaml"
        )
    print(
